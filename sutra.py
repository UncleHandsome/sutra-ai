...
用法：
  ...
  ★ 遞迴批次模式平行處理（--parallel 預設關閉 = 依序逐一處理，需手動啟用）：
  python sutra.py --recursive . --parallel 4       # ★ 4 路平行同時校對多個檔案
  python sutra.py --recursive D:/藏經目錄          # 未指定 --parallel：依序逐一處理
"""
...
class ReviewIssue:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ReviewIssue":
        ...
        idx_val = int(raw_idx) if (isinstance(raw_idx, (int, str)) and str(raw_idx).isdigit()) else 0
        raw_m = data.get("merge_indices")
        merge_indices = []
        if isinstance(raw_m, (int, str)):
            raw_str = str(raw_m).strip()
            if re.search(r"[-~至到]", raw_str):
                parts = [p.strip() for p in re.split(r"[-~至到]", raw_str) if p.strip().isdigit()]
                if len(parts) == 2:
                    start_i, end_i = int(parts[0]), int(parts[1])
                    merge_indices = list(range(min(start_i, end_i), max(start_i, end_i) + 1))
            elif re.search(r"[,、，及與和\s]", raw_str):
                merge_indices = [int(p) for p in re.split(r"[,、，及與和\s]+", raw_str) if p.isdigit()]
            elif raw_str.isdigit():
                merge_indices = [int(raw_str)]
        elif isinstance(raw_m, list) and raw_m:
            for i in raw_m:
                if isinstance(i, int):
                    merge_indices.append(i)
                elif isinstance(i, str) and i.strip().isdigit():
                    merge_indices.append(int(i.strip()))
        return cls(
            index=idx_val,
            issue_type=data.get("type") or data.get("issue_type") or "",
            problem=data.get("problem") or "",
            merge_indices=merge_indices or [idx_val],
...
class ApiKeyPool:
    ...
    def __init__(self, keys: List[str]):
        ...
        self.current_idx = 0
        self.last_used: Dict[str, float] = {k: 0.0 for k in self.all_keys}
    ...
    def next_key_for_request(
        ...
        """★ 每一個 Request 輪流切換至下一把 Key，並對該 Key 進行獨立冷卻控制"""
        if not self.active_keys:
            return ""
        self.current_idx = (self.current_idx + 1) % len(self.active_keys)
        new_key = self.active_keys[self.current_idx]

        # 獨立 Key 冷卻控制（避免單把 Key 短時間內打滿 RPM）
        if target_cooldown > 0.0:
            last_time = self.last_used.get(new_key, 0.0)
            now = time.time()
            elapsed = now - last_time
            if elapsed < target_cooldown:
                wait_sec = target_cooldown - elapsed
                if logger:
                    logger.info(f"  ⏳ [金鑰冷卻] Key ({self.mask_key(new_key)}) 冷卻保護中，等待 {wait_sec:.1f} 秒...")
                time.sleep(wait_sec)

        self.last_used[new_key] = time.time()
    ...
    def _apply_key_to_client(self, client: Any, key: str) -> None:
        """深層穿透更新 OpenAI Client 實例的 API Key、default_headers 與底層 httpx 授權"""
        if hasattr(client, "api_key"):
            client.api_key = key
        # 同步更新 OpenAI SDK v1.x 的 default_headers
        ...
        if hasattr(client, "_client"):
            try:
                ...
                if hasattr(client._client, "default_headers") and isinstance(client._client.default_headers, dict):
                    client._client.default_headers["Authorization"] = f"Bearer {key}"
            except Exception:
                pass
...
# ★ 全域統一 Markdown 段落切分正則（嚴格錨定行首標題結構與冒號/引號/閉括號，防止正文引用誤切斷）
SECTION_SPLIT_REGEX = re.compile(
    r"(?:\n\s*---\s*\n|(?<=\n)(?=(?:[\s#*`>]*【當前經文進度】|"
    r"[\s#*`>]*【單句銷文】|(?:[\s#*`>]*🔹|【\s*🔹?\s*|#{1,4}\s*(?:🔹\s*)?)[\s*`>]*原典[\s*`_】\]\)]*[：:\s\n「『\"'“])))"
)

# 統一古異體字至標準通行字（單向歸一化至大藏經通行正體字）
...
def clean_markdown_content(raw_content: str) -> str:
    ...
    # 0. 徹底移除各類推理模型（DeepSeek, Qwen, GLM, Claude 等）的思維鏈區塊
    text = RE_THINK_TAG.sub("", text).strip()
    # 清理 <think>, <thought>, <reasoning> 各類閉合與未閉合思考標籤
    text = re.sub(r"<(?:think|thought|reasoning)>[\s\S]*?(?:</(?:think|thought|reasoning)>|$)", "", text, flags=re.IGNORECASE).strip()
    text = re.sub(r"^[\s\S]*?</(?:think|thought|reasoning)>", "", text, flags=re.IGNORECASE).strip()
    text = re.sub(r"^:::+\s*(?:thought|reasoning)[\s\S]*?:::+\s*", "", text, flags=re.IGNORECASE).strip()

    # 0.5 優先移除外層 Markdown 程式碼區塊標記（確保後續 ^ 錨點匹配正常）
    text = RE_CODE_FENCE_OPEN.sub("", text).strip()
    text = RE_CODE_FENCE_CLOSE.sub("", text).strip()
    ...
    # 2. 徹底移除【當前經文進度】
    text = re.sub(
        r"(?:\A|(?<=\n))[\s#*`>]*【當前經文進度】[\s\S]*?(?=(?:[\s#*`>]*(?:【單句銷文】|🔹|【\s*🔹?\s*原典|\*{1,3}\s*🔹?\s*原典|(?:^|\n)[\s#*`>]*原典[：:])|$))",
        "",
        text,
        flags=re.IGNORECASE,
    ).strip()
    text = re.sub(r"(?:\A|(?<=\n))[\s#*`>]*【單句銷文】[：:]?\s*", "", text, flags=re.IGNORECASE).strip()
    ...
    # 4. ★ 安全移除結尾【下一句預告】、狀態標籤及 AI 結尾客套話
    text = re.sub(r"(?:\n|^)[\s#*`>]*【\s*下一句(?:經文)?(?:預告)?\s*】[\s\S]*$", "", text, flags=re.IGNORECASE)
    text = re.sub(r"(?:\n|^)[\s#*`>]*🔹\s*下一句(?:經文)?(?:預告)?[：:\s\n][\s\S]*$", "", text, flags=re.IGNORECASE)
    text = re.sub(r"(?:\n|^)[\s#*`>]*(?:#{1,4}\s*)?(?:下一句(?:經文)?預告|下一句預告)[：:\s\n][\s\S]*$", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\[STATUS:\s*[^\]]*\][\s\S]*$", "", text, flags=re.IGNORECASE)
    text = re.sub(r"(?:\n|^)請問是否繼續銷文下一句[？?]?[\s\S]*$", "", text)
    # 通用過濾文末的口語總結括號（如「（以上銷文完畢）」、「（本句解析結束）」等）
...
def extract_sentence(raw_content: str) -> Optional[str]:
    """精確提取『🔹 原典』中的文字（相容各種 Markdown 粗體、標題與夾註格式，保護書名號與專有名詞）"""
    def _clean_s(t: str) -> str:
        t = t.strip()
        for _ in range(3):
            # 1. 剝除 Markdown 圍欄與行首引用符
            t = re.sub(r"^[`*_>\s]+|[`*_\s]+$", "", t).strip()
            # 1.5 清理開頭殘留之非經文編輯標記（如 段首、句首、本句 等，保護經文原有的「第某句」序數）
            t = re.sub(r"^(?:(?:段首|段末|句首|本段|本句)[：:「『\"'“\s]*)", "", t).strip()
            # 2. 僅剝除外層對話引號（嚴禁誤殺書名號《》與夾註括號）
            t = re.sub(r"^[「『\"'“‘]+|[」』\"'”’]+$", "", t).strip()
            # 3. 清理開頭多餘的行首標點（保留合法書名號《）
            t = re.sub(r"^[，。！？；、：）\)\]】〕＞>\s]+", "", t).strip()
            # 4. 清理末尾懸空的未閉合前括號
            t = re.sub(r"[（\(\[【〔＜\s]+$", "", t).strip()
        return t

    # 增強版正則：相容 **🔸 釋詞**、### 🔸 釋詞 等各類前綴標題，防止原典吞噬後續段落
    lookahead_boundary = r"(?=(?:\n[\s*`>_]*[🔸🔹]|\n[\s*`>_]*【|\n[\s*`>_]*#|$))"

    primary_pat = (
        r"(?:[\s#*`>_]*🔹[\s*`>_]*|【\s*🔹?\s*|\*{1,3}\s*🔹\s*)"
        r"(?:\*\*|__)?原典(?:\*\*|__)?"
        r"(?:[（\(][^）\)]*?[）\)])?"
        r"(?:[\s*`>_]*[：:]|[：:][\s*`>_]*|】[：:]?|\*{1,3}[：:]?|\s*\n|\s+(?=[「『\"'“‘《〈【〔（(]))"
        rf"\s*([\s\S]*?){lookahead_boundary}"
    )
    ...
    fallback_patterns = [
        rf"(?:[\s#*`>]*原典[\s*`>]*[：:\n]|[【［]\s*原典\s*[】］][：:]?)\s*([\s\S]*?){lookahead_boundary}",
        rf"(?:[\s#*`>]*經文[\s*`>]*[：:\n]|[【［]\s*經文\s*[】］][：:]?)\s*([\s\S]*?){lookahead_boundary}",
    ]
...
def is_ignorable_gap(gap_text: str) -> bool:
    """判斷是否為純符號空白、經名品題、譯者署名或科判序號/標籤（具備防誤殺保護）"""
    clean = normalize_text(gap_text)
    if not clean:
        return True

    # 1. 嚴格品題、卷次結構與獨立經名/論名（排除包含「受持/當知/名為」等正文動詞）
    strict_title_patterns = [
        ...
        # 移除寫死經名，通用支援 2~30 字無句讀之獨立經/論/律/品題名
        r"^[^，。！？；、：—…\n]{2,30}(?:經|論|律|疏|科判|義貫)(?:第[一二三四五六七八九十\d]+)?$",
    ]

    # 2. 常見三藏譯師署名與經首序文題記（嚴格錨定朝代/譯師公式，避免誤殺『法師/說/集』等經文正文）
    translator_patterns = [
        r"^(?:後漢|曹魏|東晉|西晉|符秦|姚秦|北涼|劉宋|蕭齊|陳|北齊|後周|隋|唐|宋|元|明|清)?\s*(?:安息國|月支|月氏|天竺|中天竺|西域|北印度|南印度|東印度)?\s*(?:三藏法師|譯經沙門|沙門|大德|朝散大夫)?\s*[\u4e00-\u9fa5]{2,6}\s*(?:奉詔|奉敕|奉制|同|共)?\s*(?:譯|重譯|續譯|筆受|譯語|製)$",
        r"^(?:御製|皇帝製|唐太宗|高宗|武后|唐三藏)[\u4e00-\u9fa5\w\s]{2,15}?(?:序|讚|文|記)$",
        r"^(?:開經偈|無上甚深微妙法.*|百千萬劫難遭遇.*|我今見聞得受持.*|願解如來真實義.*)$",
    ]

    # 3. 古籍科判句段序號、條目標記與論議導語（極短 <= 12 字，純屬結構標記非獨立教理）
    rubric_patterns = [
        # 句段序號：如「第一句」、「第二句」、「初句」、「次句」、「第一段」、「第三門」、「其二」等
        r"^(?:第?[一二三四五六七八九十百千\d]+[句段門條點冊次會分卷品篇節折場案事]|(?:初|次|後|末)[句段門條會分]|其[一二三四五六七八九十\d]+)(?:者)?$",
        # 論義導語標記：如「問曰」、「答曰」、「釋曰」、「論曰」、「經言」、「偈曰」、「次說偈言」等
        r"^(?:(?:初說|次說|後說|重說|略說|廣說)?(?:偈言|偈曰|說偈言|說偈曰|重說偈言|重說偈曰|如是偈言|爾時偈言|說偈)|何謂為[義字]|此明何義|應知何義|依何等法|云何得知|問曰|答曰|釋曰|論曰|經曰|經言|經云|論云|如是應知)+$",
    ]
    ...
    if len(clean) <= 35:
        ...
        if any(re.match(pat, clean) for pat in translator_patterns):
            return True
    if len(clean) <= 12:
        if any(re.match(pat, clean) for pat in rubric_patterns):
            return True
...
def validate_output_format(raw_content: str) -> Tuple[bool, List[str]]:
    """驗證輸出格式是否包含全部必備結構（嚴格鎖定行首標題結構，杜絕正文名詞觸發假陽性）"""
    required_sections = [
        (
            "🔹 原典",
        ...
        (
            "🔸 釋詞",
            r"(?:🔸\s*(?:\*\*|#)*\s*(?:釋詞|釋義|詞義|字詞|名相)|【\s*(?:釋詞|釋義|詞義|字詞解釋|名相解釋|字句解釋)\s*】|^#{1,4}\s*.*(?:釋詞|釋義|詞義|字詞|名相)|^[\s*`-]*\*{0,2}(?:釋詞|釋義|詞義|字詞解釋|名相解釋|名相釋義)\*{0,2}[：:])"
        ),
        (
            "🔸 銷文",
            r"(?:🔸\s*(?:\*\*|#)*\s*(?:銷文|消文|語譯|白話|白話語譯|經文消文)|【\s*(?:銷文|消文|語譯|白話語譯|白話解說|經文銷文)\s*】|^#{1,4}\s*.*(?:銷文|消文|語譯|白話)|^[\s*`-]*\*{0,2}(?:銷文|消文|語譯|白話語譯|白話銷文|經文語譯)\*{0,2}[：:])"
        ),
        (
            "【詳解】",
            r"(?:【\s*(?:詳解|詳細解析|義理詳解|經文詳解|深度剖析|解析|義理解析|詳細解說|經文剖析)\s*】|^#{1,4}\s*.*(?:詳解|解析|剖析|義理解析)|^[\s*`>-]*\*{1,2}(?:詳解|詳細解析|經文詳解|深度剖析|義理解析|詳細解說)\*{1,2}[：:]?|^[\s*`>-]*(?:詳解|詳細解析|經文詳解|深度剖析|義理解析|詳細解說)[：:])"
        ),
        (
            "【義理通解】",
            r"(?:【\s*(?:義理通解|義理闡釋|義理闡述|義理發微|義理貫通|義理|通解|教理通解|義理總結|實修啟發|心性啟發)\s*】|^#{1,4}\s*.*(?:義理通解|義理闡釋|義理闡述|義理|通解|實修啟發|心性啟發)|^[\s*`>-]*\*{1,2}(?:義理通解|義理闡述|義理闡釋|義理發微|義理貫通|義理|通解|教理通解|義理總結|實修啟發)\*{1,2}[：:]?|^[\s*`>-]*(?:義理通解|義理闡述|義理闡釋|義理發微|義理貫通|教理通解|義理總結|實修啟發)[：:])"
        ),
    ]
...
def verify_sentence_quality(
    ...
    # 3. 起點對齊檢查（具備發語詞/重複主詞/短起點智慧容錯，並容許底本缺字符號與校勘位移）
    match_pos = clean_rem.find(clean_s_text) if clean_s_text else -1
    min_check_len = min(len(clean_s_text), len(clean_rem), 4)
    prefix_match = (
        min_check_len > 0
        and clean_s_text[:min_check_len] == clean_rem[:min_check_len]
    )

    # 動態容許 1 字增刪位移（如 AI 自動填補古籍闕字或底本脫漏校勘標記）
    check_w = min(len(clean_s_text), len(clean_rem), 6)
    direct_common = sum(1 for a, b in zip(clean_s_text[:check_w], clean_rem[:check_w]) if a == b) if check_w > 0 else 0
    shift_common_1 = sum(1 for a, b in zip(clean_s_text[1:check_w], clean_rem[:check_w - 1]) if a == b) if check_w >= 3 else 0
    shift_common_2 = sum(1 for a, b in zip(clean_s_text[:check_w - 1], clean_rem[1:check_w]) if a == b) if check_w >= 3 else 0
    is_fuzzy_start = (check_w >= 3 and (max(direct_common, shift_common_1, shift_common_2) / (check_w - 1 if max(shift_common_1, shift_common_2) > direct_common else check_w)) >= 0.65)

    if match_pos != 0 and not prefix_match and not is_fuzzy_start:
        rem_prefix_6 = clean_rem[: min(6, len(clean_rem))]
        s_pos = clean_s_text.find(rem_prefix_6) if rem_prefix_6 else -1
        if 0 <= s_pos <= 10:
            pass  # AI 完美對齊起點或稍帶前導詞，放行
        elif match_pos > 0:
            skipped_chars = clean_rem[:match_pos]
            # 通用辨識：大藏經冊頁號 (001a01)、版勘標籤、中文序號 (第某句/段)、段首句首等編輯標記
            editorial_pat = r"^(?:[0-9a-zA-Z_]+|[一二三四五六七八九十百千甲乙丙丁]+|第|句|段|門|條|次|初|後|其|篇|節|品|卷|問|答|曰|云|言|宋|元|明|清|磧砂|高麗|宮|聖|藏|大正|嘉興|磧|段首|段末|句首|本段|原典|經文)+$"
            is_editorial_marker = (len(skipped_chars) <= 15 and bool(re.match(editorial_pat, skipped_chars)))
            if is_dup_context or is_editorial_marker:
                pass
            else:
...
def request_and_validate_segment(
    ...
    max_retries = max(len(pool) * 2, 6) if (pool and pool.has_multiple()) else 5
...
def get_sutra_coverage(sutra_text: str, completed_sentences: List[str]) -> Tuple[List[int], str, List[bool], Dict[int, Tuple[int, int]]]:
    ...
    for orig_idx, s, s_norm in sorted_sentences:
        ...
        if not matches and len(s_norm) >= 4:
            cand_matches = []
            step = max(1, min(4, len(s_norm) // 4))
            for offset in range(0, max(1, len(s_norm) - 3), step):
                seed = s_norm[offset : offset + 4]
                for m in re.finditer(re.escape(seed), norm_sutra):
                    cand_pos = m.start() - offset
                    if 0 <= cand_pos <= n - 4 and cand_pos not in cand_matches:
                        target_win = norm_sutra[cand_pos : min(n, cand_pos + len(s_norm))]
                        matching_chars = sum(1 for a, b in zip(s_norm, target_win) if a == b)
                        if (matching_chars / len(s_norm)) >= 0.60:
                            cand_matches.append(cand_pos)
                if cand_matches:
                    break
            if cand_matches:
        ...
        for pos in matches:
            ...
            if is_prefix_fallback and len(s_norm) > 0:
            ...
            # 綜合評分：新覆蓋字數獎勵 - 超遠距離偏差懲罰
            dist_penalty = (dist / max(1, n)) * 15.0
            score = new_cover - dist_penalty
            if score > max_score or (score == max_score and dist < best_distance):
                max_score = score
                best_distance = dist
                best_pos = pos
...
def find_missing_gaps(sutra_text: str, completed_sentences: List[str]) -> List[SutraGap]:
    ...
    pos_to_seg_idx = {}
    # 按段落索引由小到大有序建立位置映射，確保前文錨點具備嚴格單調性
    for s_idx in sorted(sentence_slots.keys()):
        start_p, end_p = sentence_slots[s_idx]
    ...
    while i < n:
        if not covered_mask[i]:
            ...
            if not is_ignorable_gap(gap_raw) and len(normalize_text(gap_raw)) >= 1:
                # 向前回溯尋找最近的有效段落錨點
                anchor_prev_idx = -1
                for p in range(gap_start_clean - 1, -1, -1):
                    if p in pos_to_seg_idx:
                        anchor_prev_idx = pos_to_seg_idx[p]
                        break

                if gap_start_clean == 0 or anchor_prev_idx == -1:
                    pos_type = "head"
                    prev_idx = -1
                elif gap_end_clean == n:
                    pos_type = "tail"
                    prev_idx = anchor_prev_idx if anchor_prev_idx != -1 else (len(completed_sentences) - 1)
                else:
                    pos_type = "middle"
                    prev_idx = anchor_prev_idx
...
def get_source_slice(
    ...
    if not sentence_slots and not force_head and not force_tail:
        return "".join(target_segments)
    ...
    if force_tail:
        ...
        next_p = -1
        for n_i in range(end_seg_idx + 1, len(segments)):
            if n_i in sentence_slots:
                next_p = sentence_slots[n_i][0]
                break

        expected_len = sum(len(normalize_text(s)) for s in target_segments)
        if next_p != -1 and next_p >= start_clean_pos:
            end_clean_pos = next_p
        else:
            end_clean_pos = min(len(clean_to_raw_map), start_clean_pos + max(expected_len, 20))

    expected_len = sum(len(normalize_text(s)) for s in target_segments)
    if end_clean_pos <= start_clean_pos:
        if force_tail:
            end_clean_pos = len(clean_to_raw_map)
        else:
            end_clean_pos = min(len(clean_to_raw_map), start_clean_pos + max(expected_len, 10))
    elif not force_head and not force_tail and (end_clean_pos - start_clean_pos) > max(expected_len * 2 + 60, expected_len + 120):
        end_clean_pos = min(len(clean_to_raw_map), start_clean_pos + max(expected_len, 10))
    ...
    elif end_clean_pos > 0 and (end_clean_pos - 1) < len(clean_to_raw_map):
...
def find_best_position(raw_target: str, section_text: Optional[str], clean_sutra: str, norm_sutra: str, last_pos: int = 0) -> int:
    """【上下文消歧義定位器】具備異體字容錯、單調進度鎖定與 Tri-gram 重疊度比對"""
    if not raw_target or not clean_sutra or not norm_sutra:
        return -1
    clean_target = RE_CLEAN_CJK.sub("", raw_target)
    norm_target = normalize_text(raw_target)
    if not clean_target or not norm_target:
        return -1
    sec_text_str = section_text or ""
    ...
    if not positions and len(norm_target) >= 3:
        # 通用滑動種子探測（無硬編碼）：改以 3 字種子探測，容許短句內部存在單一異體字或缺字符號
        target_len = len(norm_target)
        n_len = len(norm_sutra)
        seed_len = min(3, target_len)
        step = max(1, min(3, target_len // 3))
        cand_scores = {}

        for offset in range(0, max(1, target_len - seed_len + 1), step):
            seed = norm_target[offset : offset + seed_len]
            s_start = 0
            while True:
                s_idx = norm_sutra.find(seed, s_start)
                if s_idx == -1:
                    break
                cand_pos = s_idx - offset
                if 0 <= cand_pos <= n_len - 4 and cand_pos not in cand_scores:
                    win = norm_sutra[cand_pos : min(n_len, cand_pos + target_len)]
                    match_count = sum(1 for a, b in zip(norm_target, win) if a == b)
                    ratio = match_count / target_len
                    if ratio >= 0.55:  # 字符吻合率達 55% 以上即判定為同一經文位置
                        cand_scores[cand_pos] = ratio
                s_start = s_idx + 1

        if cand_scores:
            max_ratio = max(cand_scores.values())
            positions = [p for p, r in cand_scores.items() if r >= max_ratio - 0.1]
    ...
    max_score = -float('inf')
    sec_norm = normalize_text(sec_text_str)
    ...
    for pos in positions:
        ...
        # 連續性加權：窗口大小自適應為當前目標字數的 1.5 倍（至少 50 字，最大 200 字）
        continuity_window = max(50, min(200, int(len(norm_target) * 1.5)))
        if last_pos > 0 and 0 <= (pos - last_pos) <= continuity_window:
            score += 50.0
        ...
        if len(sec_norm) > len(norm_target) + 6:
            for i in range(0, len(context_window) - 2):
                if context_window[i : i + 3] in sec_norm:
                    score += 1
            # 加重超遠距離懲罰（防止常見名相干擾導致跨章節飛躍）
            score -= (dist / 25.0)
        else:
            score = -dist

        if score > max_score or (score == max_score and dist < abs(best_pos - last_pos)):
            max_score = score
            best_pos = pos
...
def safe_write_file(filepath: str, content: str) -> None:
    ...
    # 確保目標父目錄存在，防止指定子目錄時 FileNotFoundError
...
def extract_segments_from_md(filepath: str) -> List[str]:
    """從 MD 檔案中依序提取出所有原典段落（與 parse_md_sections 及 extract_sentence 100% 同步）"""
    if not os.path.exists(filepath):
        return []
    _, sections = parse_md_sections(filepath)
    results = []
    invalid_tokens = {"無", "（無）", "(無)", "none", "null", ""}
    for sec in sections:
        sent = extract_sentence(sec)
        if sent and sent not in invalid_tokens and len(normalize_text(sent)) >= 1:
            results.append(sent)
    return results
...
def parse_md_sections(filepath: str) -> Tuple[str, List[str]]:
    ...
    invalid_tokens = {"無", "（無）", "(無)", "none", "null", ""}
    def _is_valid_section(blk: str) -> bool:
        sent = extract_sentence(blk)
        return bool(sent and sent not in invalid_tokens and len(normalize_text(sent)) >= 1)
    ...
    for b in raw_sections:
        ...
        if not sections:
            if _is_valid_section(b):
                sections.append(b)
            else:
                header = (header.strip() + "\n\n" + b).strip() + "\n\n"
        else:
            if not _is_valid_section(b):
                sections[-1] += "\n\n" + b
            else:
                sections.append(b)
...
def reorder_markdown_by_sutra(md_content: str, sutra_text: str) -> str:
    ...
    # 完整截取直到第一個 --- 分隔線或第一個原典段落前的所有標題與前導文字
    first_split = SECTION_SPLIT_REGEX.search(content_body)
    if first_split and not extract_sentence(content_body[:first_split.start()]):
        header = content_body[:first_split.start()].strip()
        content_body = content_body[first_split.end():].strip()
    elif title_match := re.match(r"^(#{1,6}\s+[^\n]+(?:\n+[>*\s\w\u4e00-\u9fa5（\(\[【〔<《〈][^\n]*)*\n*)", content_body):
    ...
    for item in sorted_by_len:
        ...
        if is_prefix_fallback and matches:
            valid_matches = []
            for pos in matches:
                target_window = norm_sutra[pos: min(pos + len(norm_s), n)]
                ratio = difflib.SequenceMatcher(None, norm_s, target_window).ratio()
                if ratio >= 0.60:
        ...
        expected_pos = int((item.get("orig_idx", 0) / max(1, len(section_items))) * n)
        best_pos = matches[0]
        min_score = float("inf")
        for pos in matches:
            ...
            dist = abs(pos - expected_pos)
            # 綜合評分：槽位重疊度懲罰 + 相對順序距離懲罰，徹底防止相同排比句全部擠在第一個槽位
            score = overlap * 100.0 + (dist / max(1, n)) * 20.0
            if score < min_score:
                min_score = score
                best_pos = pos
        ...
        item["best_pos"] = best_pos

    # 為未匹配段落就近吸附錨定前段槽位，保持原始文脈連續，嚴禁線性比例散佈全篇
    first_known_pos = next((it["best_pos"] for it in section_items if it.get("best_pos", -1) != -1), 0)
    curr_pos = first_known_pos
    for it in section_items:
        if it.get("best_pos", -1) != -1:
            curr_pos = it["best_pos"]
            it["pos"] = curr_pos
        else:
            it["pos"] = curr_pos
        assigned_sections.append(it)
    ...
    for item in assigned_sections:
        ...
        # ★ 安全去重準則：僅當同槽位且「整段銷文內容完全一致」時才去重，保護連續出現的相同經句獨立銷文
        sec_pos_key = (pos, sec_hash)
        if sec_pos_key not in seen_sec_hashes:
            seen_sec_hashes.add(sec_pos_key)
    ...
    for new_content, merge_idx in corrections:
        ...
        for orig_i in merge_idx:
            if 0 <= orig_i < len(segments_snapshot):
                ...
                # 優先在目標原始索引周圍尋找，加入距離懲罰，防止同名定型句跳回卷首
                best_sec_idx = -1
                min_dist_score = float('inf')
                for sec_idx, sec_text in enumerate(sections):
                    if sec_idx in target_indices_in_sections:
                        continue
                    ...
                    # 動態距離閾值：依總段落數自適應（最小 15 段，最大不超過 10% 總段數）
                    max_allowed_dist = max(15, len(sections) // 10)
                    dist = abs(sec_idx - orig_i)
                    if norm_target == sec_norm:
                        score = dist
                        if score < min_dist_score:
                            min_dist_score = score
                            best_sec_idx = sec_idx
                    elif (norm_target in sec_norm or sec_norm in norm_target) and dist <= max_allowed_dist:
                        score = dist + 50
                        if score < min_dist_score:
                            min_dist_score = score
                            best_sec_idx = sec_idx
...
def handle_api_exception(
    ...
    if pool is not None and pool.has_multiple():
        pool.rotate_client(client, logger, reason="觸發頻率限制 (429/RPM)")
        if retry_delay > 0:
...
def parse_review_json_response(raw_text: str, max_idx: int) -> List[ReviewIssue]:
    ...
    # 轉換並過濾非法/越界索引（深度防禦字串拆字 Bug）
    valid_issues: List[ReviewIssue] = []
    for raw_iss in issues_raw:
        if not isinstance(raw_iss, dict):
            continue
        raw_merge = raw_iss.get("merge_indices")
        if raw_merge is None:
            raw_merge = [raw_iss.get("index", -1)]
        elif isinstance(raw_merge, (int, str)):
            raw_str = str(raw_merge).strip()
            # 支援中英文範圍符：- ~ 至 到 -> 轉換為連續整數 list
            if re.search(r"[-~至到]", raw_str):
                parts = [p.strip() for p in re.split(r"[-~至到]", raw_str) if p.strip().isdigit()]
                if len(parts) == 2:
                    start_i, end_i = int(parts[0]), int(parts[1])
                    raw_merge = list(range(min(start_i, end_i), max(start_i, end_i) + 1))
                else:
                    raw_merge = [int(parts[0])] if parts else [-1]
            # 支援中英文枚舉分隔符：, 、 及 與 和 空格
            elif re.search(r"[,、，及與和\s]", raw_str):
                raw_merge = [int(p) for p in re.split(r"[,、，及與和\s]+", raw_str) if p.isdigit()]
            elif raw_str.isdigit():
                raw_merge = [int(raw_str)]
            else:
                raw_merge = [-1]
...
def stream_completion(
    ...
    """封裝串流 LLM 呼叫：全平台強制開啟深度思考 (Thinking / Reasoning)"""
    ...
    # 1. 精準適配各模型最大輸出 Token 規格 (全部檔位拉至極限 MAX)
    m_lower = model.lower()
    if "dots" in m_lower:
    ...
    # ============================================================
    # ★★★ 全平台強制開啟深度思考 (Thinking Mode) 智慧適配 ★★★
    # ============================================================
    effort_val = reasoning_effort or "high"
    base_url_str = str(client.base_url).lower()

    if "googleapis" in base_url_str or "gemini" in m_lower:
        # 🟢 1. Google Gemini：開啟思考 (reasoning_effort) + 關閉安全攔截
        create_kwargs["reasoning_effort"] = effort_val
        ...
        # 🟢 2. OpenRouter (GLM-5 / Qwen / Claude 等)：使用 OpenRouter 標準 reasoning 格式 (強制 Max 思考)
        create_kwargs["extra_body"] = {
            "reasoning": {
                "effort": "max" if effort_val in ["high", "max"] else effort_val,
                "exclude": False
            }
        }
    else:
        # 🟢 3. DeepSeek 官方 / OpenCode / Zen / NVIDIA NIM：雙通道思考拉至最高檔位
        create_kwargs["reasoning_effort"] = effort_val
        create_kwargs["extra_body"] = {
            "thinking": {"type": "enabled"},
            "reasoning": {"effort": "max"},
            "reasoning_effort": "max" if effort_val in ["high", "max"] else effort_val,
        }

    # 4. 發送請求與智慧降級保護（若特定端點報錯則自動平滑降級）
    try:
        resp = client.chat.completions.create(**create_kwargs)
    except Exception as api_err:
        err_str = str(api_err).lower()
        # 降級處理 1: max_tokens 過大
        if "max_tokens" in err_str or "maximum allowed" in err_str:
            create_kwargs["max_tokens"] = 4096
        # 降級處理 2: 某些舊模型不支援 reasoning_effort 或 extra_body
        if "reasoning_effort" in err_str or "extra_body" in err_str or "unrecognized" in err_str:
            create_kwargs.pop("reasoning_effort", None)
            create_kwargs.pop("extra_body", None)
        # 降級處理 3: 移除 stream_options
    ...
    for chunk in resp:
        ...
        if chunk.choices and len(chunk.choices) > 0:
            ...
            if finish_reason:
                detected_finish_reason = finish_reason
            ...
            # 只要任何模型進入思考階段，立即印出提示
            if reasoning_val and isinstance(reasoning_val, str) and reasoning_val.strip() and not thinking_logged:
                thinking_logged = True
                logger.info(f"  💭 [模型深度推理中 ({effort_val})...]")
    ...
    if detected_finish_reason in ["length", "MAX_TOKENS"]:
        logger.warning("  ⚠️ [輸出截斷警告] 模型輸出觸及 max_tokens 上限！")
    elif detected_finish_reason in ["safety", "SAFETY", "content_filter"]:
        logger.warning("  ⚠️ [安全過濾警告] 觸發後端安全過濾機制！")
...
def advance_text_pointer(remaining_text: str, extracted_sentence: str) -> str:
    ...
    # 6 ➔ 5 ➔ 4 ➔ 3 字階梯退避尋找後綴錨點，防止末尾標點或字符出入導致指針失配
    for s_try in range(min(6, len(norm_sent)), 2, -1):
        norm_suffix = norm_sent[-s_try:]
        start_search = 0
        cand_matches = []
        while True:
            ...
            if idx == -1:
                break
            cand_matches.append(idx)
            start_search = idx + 1
        if cand_matches:
            suffix_matches = cand_matches
            actual_suffix_len = s_try
            break
    ...
    if suffix_matches:
        ...
        best_norm_end = min(
            suffix_matches,
            key=lambda p: abs((p + actual_suffix_len) - expected_end)
        ) + actual_suffix_len
    ...
    if cut_idx != -1:
        if cut_idx < len(remaining_text):
            ...
            # 循環清除開頭的所有標點、引號、空括號對 ()/（）/[]/【】 與孤立開閉括號
            while True:
                prev_len = len(remaining_text)
                # 1. 移除成對的空括號或空校勘符號
                remaining_text = re.sub(r"^(?:\(\s*\)|（\s*）|\[\s*\]|【\s*】|〔\s*〕|〈\s*〉|《\s*》)", "", remaining_text).strip()
                # 2. 移除開頭的各類標點、括號、引號、格式符號與空白
                remaining_text = re.sub(r"^[，。！？；、：—…\(\)（）\[\]【】《》〈〉「」『』\"\'“”‘’◎\<\>#*`_\s　]+", "", remaining_text).strip()
                if len(remaining_text) == prev_len:
                    break

            # 安全防護：僅在剩餘有效純漢字確實為 0 時才判定為消化完畢（完整保留 1~2 字短真言/結語）
...
def generate_sutra_segments(
    ...
    while remaining_text and len(normalize_text(remaining_text)) > 2 and loop_guard < max_loops:
        loop_guard += 1
...
def fix_single_issue(
    ...
    # ★ 孤立標點/括號快速修復直通（本地免 API 快速清洗，嚴格校驗實質修改）
    ...
    # ★ 純位置錯位 / 自身完整快速直通（免調用 API，由物理重排引擎自動歸位）
    # 嚴格排除「斷句邊界錯位」等涉及經文標點、字句切分與內容修訂之問題
    is_boundary_or_text_issue = any(
        kw in issue_type_str or kw in problem_desc
        for kw in ["邊界", "斷句", "切分", "拆分", "腰斬", "標點", "句讀", "漏失", "句號", "逗號", "冒號", "未完", "重寫"]
    )
    has_misplacement_intent = not is_boundary_or_text_issue and (
        any(kw in issue_type_str for kw in ["位置錯置", "順序錯置", "倒置", "順序顛倒", "位置倒置", "段落錯置", "重排", "移至"])
        or any(kw in problem_desc for kw in ["序列中錯置", "位置錯置", "順序錯置", "整段移至", "移至第", "移到", "調換順序", "前後倒置", "順序顛倒"])
    )
    is_self_contained = any(kw in problem_desc for kw in [
        "自身完整", "無須合併", "無須拆分", "無需合併", "無需拆分", "不用重寫", "不需重寫",
        "整段移至", "逕移", "直接移動", "僅需調整順序", "僅調順序"
    ])
    # 僅當確實為純物理順序錯置且經文自身完整無需重寫時，才直通免除 API 調用
    if has_misplacement_intent and is_self_contained and len(valid_merge_idx) == 1:
        target_idx = valid_merge_idx[0]
        if output_path and os.path.exists(output_path):
            _, all_sections = parse_md_sections(output_path)
            if target_idx < len(all_sections):
                target_block = all_sections[target_idx]
                logger.info(
                    f"  ⚡ [本地極速重排] 段落 [{target_idx}] 自身完整（{issue_type_str}），"
                    f"將由物理重排演算法自動校準歸位，免除 API 調用！"
                )
                if on_step_done and callable(on_step_done):
                    on_step_done(valid_merge_idx, target_block)
                return target_block, valid_merge_idx

    # ★ 核心改進：聽從 AI 審查員判定，動態決定「融攝前句」、「融攝後句」或「獨立切片」
    wants_pull_prev = any(kw in problem_desc for kw in ["拉前句", "合流前段", "承前", "與前段合併", "前偈", "合併重熔"])
    wants_pull_next = any(kw in problem_desc for kw in ["拉後句", "合流後段", "啟後", "與後段合併", "後偈"])

    if is_gap_fix and gap_text and merge_segs and wants_pull_prev:
        # 🟢 AI 判定：半句承前，將前段經文完整拉進來一起重熔銷文！
        prev_seg_text = merge_segs[0].strip()
        combined_raw = f"{prev_seg_text}\n{gap_text}".strip()
        logger.info(f"  🔗 [AI 裁決合流前句] 依審查建議，已將前段 [{first_idx}] 與漏段合併為整單元重熔！")
    elif is_gap_fix and gap_text and merge_segs and wants_pull_next:
        # 🟢 AI 判定：半句啟後，將後段經文拉進來一起重熔銷文！
        next_seg_text = merge_segs[-1].strip()
        combined_raw = f"{gap_text}\n{next_seg_text}".strip()
        logger.info(f"  🔗 [AI 裁決合流後句] 依審查建議，已將漏段與後段 [{last_idx}] 合併為整單元重熔！")
    elif is_gap_fix and len(valid_merge_idx) == 1 and gap_text and not wants_pull_prev:
        # 漏段無論多長，因寫回檔案時會佔用 first_idx 槽位，必須與前段 [first_idx] 安全合流重熔，杜絕舊段落被 pop 覆蓋丟失
        prev_seg_text = merge_segs[0].strip()
        combined_raw = f"{prev_seg_text}\n{gap_text}".strip()
        logger.info(f"  🔗 [安全合流防刪] 為防止原段落 [{first_idx}] 被覆蓋丟失，已自動與漏段合併為整單元推進！")
    elif merge_segs:
        ...
        # 若切片未包含漏段，將漏段安全拼入
        if gap_text and normalize_text(gap_text) not in normalize_text(combined_raw):
    ...
    # 確保清除殘留的編輯註記前綴（如 段首、句首 等）
    combined_raw = re.sub(r"^(?:(?:第[一二三四五六七八九十\d]+[段句]|段首|段末|句首|本段|本句)[：:「『\"'“\s]*)", "", combined_raw).strip()

    # ★ 動態擴展覆蓋區間：僅在長經文（>= 25 字）且後續段落長度充足（>= 15 字）確實完全包含時才擴展，防止誤吞短排比正文
    norm_comb = normalize_text(combined_raw)
    expanded_merge_idx = list(valid_merge_idx)
    scan_start = (valid_merge_idx[-1] + 1) if valid_merge_idx else 0
    for next_i in range(scan_start, len(segments)):
        seg_norm = normalize_text(segments[next_i])
        if seg_norm and len(seg_norm) >= 15 and seg_norm in norm_comb and len(norm_comb) >= 25:
            expanded_merge_idx.append(next_i)
        else:
            break
    if len(expanded_merge_idx) > len(valid_merge_idx):
        logger.info(
            f"  🔗 [多偈文脈統攝] 待處理經文（共 {len(combined_raw)} 字）涵蓋後續段落，"
            f"自動擴展替換區間：{valid_merge_idx} ➔ {expanded_merge_idx}"
        )
        valid_merge_idx = expanded_merge_idx
    ...
    if not is_gap_fix and not gap_text:
        has_delete_intent = any(kw in problem_desc for kw in [
            ...
            "將此段移除", "應將此段移除", "直接移除", "誤植", "剔除", "經外贅段", "贅段", "多餘"
        ]) or ("贅段" in issue_type_str or "經外" in issue_type_str)
        ...
        if has_delete_intent and not has_partial_keep:
            ...
            norm_sutra = normalize_text(sutra_text)
            # ★ 縮減比對微塊至 4 字，防止短句因微小字詞差異被本地直通誤殺刪除
            chunk_len = min(4, len(norm_target))
            has_sutra_chunk = any(
                norm_target[k : k + chunk_len] in norm_sutra
                for k in range(0, max(1, len(norm_target) - chunk_len + 1), 2)
            ) if len(norm_target) >= chunk_len else (norm_target in norm_sutra)
            is_extraneous = (len(norm_target) >= chunk_len and not has_sutra_chunk)

            # 統計該文本在原文與現有段落中的出現次數，避免刪除原典合法的重複經句
            sample_prefix = norm_target[: min(15, len(norm_target))]
            sutra_count = norm_sutra.count(sample_prefix) if len(sample_prefix) >= 6 else 1
            total_seg_count = sum(1 for s in segments if sample_prefix in normalize_text(s)) if len(sample_prefix) >= 6 else 1

            # 僅當現有段落數「大於」原文出現次數時，才代表有真正的多餘備份
            has_redundant_backup = (total_seg_count > sutra_count)

            # 只要確認為多餘贅出，或原文根本沒有此經文，才允許直接標記刪除
            if has_redundant_backup or is_extraneous:
                reason = "經外贅段" if is_extraneous else "重複段落"
                logger.info(f"  🗑️ 判定為{reason}，直接標記刪除：段落 {valid_merge_idx}")
                if on_step_done and callable(on_step_done):
                    on_step_done(valid_merge_idx, "<!-- DELETE -->")
                return "<!-- DELETE -->", valid_merge_idx
            else:
                logger.warning(
                    f"  🛡️ [防雙殺守護] 段落 {valid_merge_idx} 為原典自身合法出現（原文 {sutra_count} 次／段落 {total_seg_count} 次），"
                    f"禁止刪除以保護經文 100% 覆蓋率！"
                )
                target_idx = valid_merge_idx[0]
                if output_path and os.path.exists(output_path):
                    _, all_sections = parse_md_sections(output_path)
                    if target_idx < len(all_sections):
                        return all_sections[target_idx], valid_merge_idx
    ...
    if problem_desc:
        ...
        if m_keep:
            # 往前動態截取至前一標點或最多 40 字（涵蓋完整子句語義）
            start_pos = m_keep.start()
            lookback_pos = max(0, start_pos - 40)
            sub_clause = problem_desc[lookback_pos:start_pos]
            # 若中間有句號/分號，僅取該標點後的當前子句
            clause_parts = re.split(r"[。；\n]", sub_clause)
            current_sub_clause = clause_parts[-1]

            is_negated = any(neg in current_sub_clause for neg in ["不", "勿", "毋", "嚴禁", "無須", "無需", "不用", "免", "別", "莫", "切忌"])
            if not is_negated:
...
#  七、審查與診斷分析引擎
# ============================================================
def is_full_gatha_len(n: int) -> bool:
    """動態判定是否為四言(16)、五言(20)、六言(24)、七言(28)的整偈整倍數"""
    if n < 16:
        return False
    return any(n % (meter * 4) == 0 for meter in (4, 5, 6, 7))

def is_incomplete_gatha_fragment(n: int) -> bool:
    """動態判定是否為非整偈的韻文殘句（僅限五言 5/10/15、六言 6/12/18、七言 7/14/21 韻文；四言為通用漢譯散文對仗駢句，不可視為殘偈）"""
    if n <= 0:
        return False
    for meter in (5, 6, 7):
        if n % meter == 0 and (n % (meter * 4) != 0) and (n <= meter * 3):
            return True
    return False
...
def pre_check(sutra_text: str, segments: List[str], md_sections: Optional[List[str]] = None) -> List[ReviewIssue]:
    ...
    for i, seg in enumerate(segments):
        ...
        if pos != -1:
            ...
            if len(clean_t) >= 8:
                ...
                if pos in seen_segment_positions and count_in_sutra <= 1:
                    ...
                    dup_prev_seg = segments[dup_orig_idx].strip() if (0 <= dup_orig_idx < len(segments)) else ""
                    dup_prev_norm = normalize_text(dup_prev_seg)
                    # ★ 雙重驗證：真正的重複必須是全段內容高度實質一致（字符吻合率 >= 80%），嚴禁僅憑前 10 字前綴相同誤殺排比定型句
                    max_l = max(len(clean_t), len(dup_prev_norm))
                    common_c = sum(1 for a, b in zip(clean_t, dup_prev_norm) if a == b)
                    is_true_dup = (max_l > 0 and (common_c / max_l) >= 0.80) or (clean_t == dup_prev_norm)

                    if dup_prev_norm and is_true_dup:
        ...
        # 3. 古籍無標點相鄰文字之物理碎首檢測（結合佛學文法詞與整偈韻律智慧辨識）
        prev_start_p = seg_idx_to_pos.get(i - 1, -1) if i > 0 else -1
        prev_seg_end_pos = (prev_start_p + len(normalize_text(segments[i - 1]))) if prev_start_p != -1 else -1
        is_adjacent_to_prev = (pos != -1 and prev_seg_end_pos != -1 and abs(pos - prev_seg_end_pos) <= 1)

        if style != "NO_PUNCT" and 0 < pos < len(clean_to_raw) and i > 0 and is_adjacent_to_prev:
            ...
            if not has_sep:
                prev_clean_text = normalize_text(segments[i - 1])
                char_before = sutra_text[prev_raw_idx]

                # 判定 1：若兩段各自是標準整偈（五言 20 字、七言 28 字），古籍無標點相連完全合法
                prev_is_full_gatha = is_full_gatha_len(len(prev_clean_text))
                curr_is_full_gatha = is_full_gatha_len(len(clean_t))

                # 判定 2：檢查是否具備古文自然斷句標誌（前段以句尾詞結尾，或後段以發語詞開頭）
                has_sentence_end_marker = any(prev_clean_text.endswith(kw) for kw in ["也", "矣", "者", "乎", "哉", "耳", "已", "訖", "云", "言", "說偈言", "說偈曰"])
                has_sentence_start_marker = any(clean_t.startswith(kw) for kw in ["爾時", "佛告", "復次", "世尊", "時", "是故", "若", "彼", "此", "云何", "何以故", "唯", "善男子", "善女人", "如來", "菩薩", "問曰", "答曰", "論曰"])

                is_naturally_separated = (prev_is_full_gatha and curr_is_full_gatha) or has_sentence_end_marker or has_sentence_start_marker

                # 僅在「非自然分句」且「其中一段過短或處於詞中腰斬」時才通報合併
                if not is_naturally_separated and (len(clean_t) < 5 or len(prev_clean_text) < 5 or len(prev_clean_text) in [5, 10, 15, 7, 14, 21]):
                    if RE_CLEAN_CHAR.match(char_before):
                        issues.append(ReviewIssue(
                            index=i - 1,
                            issue_type="字詞開頭碎首腰斬",
                            problem=f"本段開頭在詞中截斷或韻文未成整偈（前字『{char_before}』無標點），應與前段合併銷文",
                            merge_indices=[i - 1, i],
                        ))
                        continue
        ...
        # 5. 物理斷尾腰斬檢測（僅在與後段在經文中實質相鄰時才判定合併）
            ...
            if pure_end:
                ...
                if last_char not in valid_puncts and pos >= 0 and clean_to_raw:
                    ...
                    if end_clean_idx < len(clean_to_raw):
                        ...
                        has_sep = bool(re.search(r"[\s\n\r　，。！？；、：—…\(\)（）\[\]【】《》〈〉「」『』\"\'“”‘’◎]", intervening))
                        if not has_sep and raw_next_idx < len(sutra_text) and RE_CLEAN_CHAR.match(sutra_text[raw_next_idx]):
                            issues.append(ReviewIssue(
                                index=i,
                                issue_type="字詞結尾斷尾腰斬",
    ...
    for g in missing_gaps:
        ...
        if pos == "head" or p_idx < 0:
            ...
            # 智慧判斷：前段是否為未完的「腰斬殘句」或「非整偈韻文」
            prev_seg = segments[p_idx] if 0 <= p_idx < total else ""
            prev_clean = normalize_text(prev_seg)
            gap_clean = normalize_text(gap_content)

            # 檢測前段是否停在未完標點、或是未湊滿整偈的殘偈（動態支援四/五/六/七言）
            prev_is_incomplete = (
                prev_seg.rstrip().endswith(("，", "、", "：", "—", "-"))
                or is_incomplete_gatha_fragment(len(prev_clean))
                or (len(gap_clean) <= 14 and not gap_content.strip().endswith(("。", "！", "？")))
            )

            if prev_is_incomplete:
                # 【情況 1：腰斬殘句】將前段、漏段與後段合流，交由 AI 重新釐定最佳斷句邊界
                target_indices = [p_idx, p_idx + 1] if p_idx + 1 < total else [p_idx]
                desc = (
                    f"段落 [{p_idx}] 結尾未完或屬半偈，與漏段（『{gap_content[:15]}...』）實質割裂，"
                    f"應合併相鄰段落由 AI 重新銷文斷句"
                )
            else:
                # 【情況 2：自足漏段】前段已完整，僅針對漏段獨立銷文並自然插入
                target_indices = [p_idx]
                desc = f"段落 [{p_idx}] 後方遺漏了獨立法義區塊 {len(gap_clean)} 字（『{gap_content[:20]}...』），應獨立補齊"
...
【核心指導哲學：預設合格、微觀自足、尊重既有科判】
佛經銷文解義的目的，是將經文切分為「最適宜開示與深入剖析的法義單元」。
★【審查最高原則（無罪推定）】：
   1. 【維持現狀優先】：若現有段落語意自足、利於逐條深入開示，【一律判定合格】，嚴禁為審查而無病呻吟！
   2. 【合併嚴格限制跨度】：合併僅用於修復「致命的文法腰斬」，`merge_indices` 【嚴禁超過 2~3 個相鄰段落】，絕對禁止一次性合併 4 個以上段落！

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🟩 一、何時【堅決放行免審（核心豁免原則）】？（重要！絕對嚴禁通報合併或拆分！）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. ★【經文原典自身之重出、重頌或排比重複 —— 堅決放行！】：
   - 若經文原文自身即有重複出現的段落（如偈頌重述長行、讚歎/校量功德重出、發願重復、定型句），每處出現皆屬原典必然結構，【必須各自獨立保留銷文，絕對嚴禁通報為「重複內容」或要求刪除】！
2. ★【經名、法名、專有名詞之受持排比（如《...》如是受持、佛名、受持句）—— 堅決放行！】：
   - 凡連續列舉之經名、品題、法門標題、佛號或受持句，每一條目皆為獨立甚深法義總綱，各段單獨銷文最利於開示深解，【絕對禁止】通報為「字詞腰斬/過度切碎」或要求跨段合併！
2. ★【外層長對話引號跨段 —— 堅決放行！】：
   - 若原典佛陀開示外層引號「...」橫跨多個平行的獨立條目/經名，各段獨立加上引號成段銷文是標準體例，【絕對禁止】因原典引號未在單句內閉合而誤判為「引號懸空/書名號割裂」！
3. ★【帶序號或法相標籤之列舉條目（一者...二者...、若...、...者）—— 堅決放行！】：
   - 凡帶數字序號或明確法相標籤之條目（如「一者、諦實故；」「捨身者，...」），無論字數多短，皆屬標準科判單元，【絕對禁止】要求合併！
4. ★【設問徵起、總標句與精簡問答（「所以者何」、「何以故」、「王言：不也。」）—— 堅決放行！】：
   - 單獨成段極利於提綱挈領與發起問端，完全合法，嚴禁通報。
5. ★【文氣連貫之中等段落（30 ~ 130 字）—— 堅決放行！】：
   - 只要內部主題集中（如連續誓願、因明推導、譬喻與法合），篇幅適中者皆屬標準段落，嚴禁過度挑刺拆分。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🟥 二、何時【才允許通報合併】？（僅限致命語法割裂 -> merge_indices 僅填 [相鄰 2~3 段]）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. 【專有名詞內部腰斬】：如前段末「阿彌陀」，後段首「佛」；或詞彙被生硬截成兩半。
2. 【純引導詞孤立成段】：如前段僅有「佛告阿難：『善男子！』」，正文全落在下一段。
3. 【條件句/假設句懸空】：僅有前置條件（如「若彼所生」），無主句、無結論，單獨無法成義。
4. 【半偈殘篇】：五言/七言韻文在非整偈處中斷（單段僅 5/7/10/14 字且句意未完）。
5. ★【漏段/半句因明裁量（極重要）】：若提供之【檢測漏段列表】中存在未銷文經文，請依佛學因明義理自主裁斷其歸屬：
   - 若為不足一偈之半偈、殘句或文義承前：【必須將前句拉進來合流】，merge_indices 填 [前段編號]，由你主導重熔銷文！
   - 若文義實質開啟後文：【拉後句合流】，merge_indices 填 [後段編號]。
   - 若法義自足獨立：merge_indices 填 [前段編號]，說明中註明獨立補齊。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✂️ 三、何時【才標記拆分】？（僅限巨大臃腫 -> merge_indices 嚴格只填 [單一段號]）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
凡單一段落過長（散文 > 140~160 字，且內部包含完全不相干的多重長篇法義轉折）時才標記拆分：
1. 【問答角色嚴重混雜】：前半段為請法者長篇發問，後半段直接轉為佛陀長篇回答。
2. 【偈頌連續堆疊過長】：偈頌連續超過 3 整偈（五言 >60 字、七言 >84 字）。
...
def ai_review(
    ...
    logger: logging.Logger,
    reasoning_effort: str = "high"
) -> Optional[List[ReviewIssue]]:
    """AI 深度審查語意連貫性（Prompt Cache 前綴對齊優化，注入漏段前後文由 AI 裁斷）"""
    ...
    # 動態掃描客觀漏段，提取前後文脈絡交由 AI 裁決
    gaps = find_missing_gaps(sutra_text, segments)
    gap_context_section = ""
    if gaps:
        gap_lines = []
        for g_i, g in enumerate(gaps, 1):
            p_i = g.prev_idx
            p_text = segments[p_i] if (0 <= p_i < len(segments)) else "（無/經首）"
            n_i = p_i + 1
            n_text = segments[n_i] if (0 <= n_i < len(segments)) else "（無/經末）"
            gap_lines.append(
                f"  [{g_i}] 漏段經文：「{g.gap_text}」（共 {len(normalize_text(g.gap_text))} 字）\n"
                f"      - 前段 [{p_i}]：「{p_text}」\n"
                f"      - 後段 [{n_i}]：「{n_text}」"
            )
        gap_context_section = (
            f"\n【🚨 待審查經文漏段列表】（共檢測出 {len(gaps)} 處未覆蓋縫隙，請由你判定應「拉前句合流」、「拉後句合流」或「獨立成段」）：\n"
            + "\n".join(gap_lines) + "\n"
        )
...
def merge_overlapping_issues(
    ...
    # ★ 關鍵保底：當原檔案為空（max_valid_len == 0）時，仍允許 index 0 的全本漏段問題通過
    effective_max = max(1, max_valid_len) if max_valid_len is not None else None

    # 1. 規範化每個 issue 為起止連續區間
    normalized_items = []
    for x in issues:
        if hasattr(x, "merge_indices"):
            raw_indices = x.merge_indices if (x.merge_indices is not None and len(x.merge_indices) > 0) else [getattr(x, "index", 0)]
            issue_type = getattr(x, "issue_type", "")
            problem = getattr(x, "problem", "")
            gap_text = getattr(x, "gap_text", None)
            position = getattr(x, "position", None)
        else:
            raw_indices = x.get("merge_indices") if (x.get("merge_indices") is not None and len(x.get("merge_indices")) > 0) else [x.get("index", 0)]
    ...
    for cur in normalized_items[1:]:
        prev = merged_raw[-1]
        # 1. 若有實質重疊且合併後總跨度不超過 3 段：安全合併；若跨度過大則獨立保留交由下一輪迭代處理
        can_merge_span = (max(prev["end"], cur["end"]) - prev["start"] + 1) <= 3
        if cur["start"] <= prev["end"] and can_merge_span:
...
# ============================================================
#  ★ 批次全域斷點續傳 (Batch Checkpoint) 輔助
# ============================================================
BATCH_CHECKPOINT_FILE = "batch_checkpoint.json"
...
def save_batch_checkpoint(data: Dict[str, Any]) -> None:
    """原子寫入批次續傳記錄"""
    try:
        temp_path = f"{BATCH_CHECKPOINT_FILE}.tmp"
        with open(temp_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        os.replace(temp_path, BATCH_CHECKPOINT_FILE)
    except Exception:
        pass

def remove_batch_checkpoint() -> None:
    """清除批次續傳記錄"""
    if os.path.exists(BATCH_CHECKPOINT_FILE):
        try:
...
def run_fill_gaps(
    ...
    while True:
        ...
        cur_gap_sig = f"{p_idx}_{normalize_text(gap_raw)}"

        # ★ 防死循環保護：若同一個 Gap 連續 2 輪完全無進展，安全中斷避免無限浪費 Token
        if cur_gap_sig == last_gap_sig:
            gap_stall_count += 1
            if gap_stall_count >= 2:
                logger.error(f"  ❌ [防死鎖保護] 漏段『{gap_raw[:25]}...』連續 2 輪推進無進展，中止此補漏任務。")
                break
        else:
            gap_stall_count = 0
            last_gap_sig = cur_gap_sig

        logger.info(f"\n📖 正在推進漏段（長度 {gap_chars_len} 字）：『{gap_raw[:30]}...』")

        new_blocks_in_gap = []
        def block_success_handler(block_content: str, sent: str):
            nonlocal total_added_blocks, prev_sentence, cur_sections, header
            total_added_blocks += 1
            prev_sentence = sent
            new_blocks_in_gap.append(block_content)
            # ★ 關鍵改善：每產出一個子單元即刻原子寫入磁碟並物理重排，中途斷線或中斷 100% 保留進度
            ...
            # 保持記憶體與磁碟物理段落同步
            parsed_header, parsed_secs = parse_md_sections(output_path)
            header = parsed_header if parsed_header else header
            cur_sections = parsed_secs if parsed_secs else cur_sections
...
def run_review(
    ...
    # ★ 核心變更：以 AI 審查員為最高準則！若 AI 針對漏段做出了裁決，優先採納 AI 的判定
    all_issues = []
    seen_indices = set()

    # 1. 優先納入 AI 深度審查檢出的所有問題（包含 AI 決定的半句合流段落）
    for iss in ai_issues:
        m_idx = tuple(iss.merge_indices if hasattr(iss, "merge_indices") else iss.get("merge_indices", [iss.get("index", -1)]))
        if len(m_idx) > 0 and m_idx[0] != -1:
            all_issues.append(iss)
            seen_indices.add(m_idx)

    # 2. 物理預檢僅作為保底：若 AI 審查遺漏了某個物理漏段，才由預檢補充
    for p_iss in pre_issues:
        p_m_idx = tuple(p_iss.merge_indices if hasattr(p_iss, "merge_indices") else p_iss.get("merge_indices", [p_iss.get("index", 0)]))
        # 檢查 AI 是否已經在該索引周圍安排了修復處理
        already_covered_by_ai = any(
            any(idx in p_m_idx for idx in ai_m)
            for ai_m in seen_indices
        )
        if not already_covered_by_ai:
            all_issues.append(p_iss)
            seen_indices.add(p_m_idx)

    # 2.5 智慧過濾：防止 AI 誤將「原典自身之重出經文」判定為重複而觸發誤刪
    norm_sutra_all = normalize_text(sutra_text)
    filtered_all_issues = []
    for iss in all_issues:
        iss_type = iss.issue_type if hasattr(iss, "issue_type") else iss.get("type", "")
        prob_desc = iss.problem if hasattr(iss, "problem") else iss.get("problem", "")
        m_indices = iss.merge_indices if hasattr(iss, "merge_indices") else iss.get("merge_indices", [iss.get("index", 0)])

        is_dup_claim = "重複" in iss_type or any(kw in prob_desc for kw in ["重複", "重出", "刪除", "贅段"])
        if is_dup_claim and m_indices:
            target_idx = m_indices[0]
            if 0 <= target_idx < len(segments):
                seg_norm = normalize_text(segments[target_idx])
                sample_key = seg_norm[: min(15, len(seg_norm))]
                if len(sample_key) >= 6:
                    sutra_occurrences = norm_sutra_all.count(sample_key)
                    segs_occurrences = sum(1 for s in segments if sample_key in normalize_text(s))
                    # 若銷文中的重複次數並未超過原文中的出現次數，代表為原典自身之重出，放行不予處理
                    if segs_occurrences <= sutra_occurrences:
                        logger.info(f"  🛡️ [放行原典重出] 段落 [{target_idx}] 為經文原典自身之重複（原文出現 {sutra_occurrences} 次），非排版錯誤，自動忽略刪除建議。")
                        continue
        filtered_all_issues.append(iss)

    # 合併重疊區間並保證升序
    merged_issue_objs = merge_overlapping_issues(filtered_all_issues, max_valid_len=len(segments))
...
def run_fix(
    ...
    corrections = []
    failed_count = 0
    # 紀錄本輪已由前方長文脈一併涵蓋替換的段落索引
    covered_by_earlier_fixes = set()

    for i, iss in enumerate(all_issues[:max_fix], 1):
        merge_tuple = tuple(sorted(list(set(iss.get("merge_indices") or [iss.get("index", 0)]))))

        # 若該段落已被前方較長文脈一併融攝修復，直接略過
        if set(merge_tuple).issubset(covered_by_earlier_fixes):
            logger.info(f"\n[{i}/{max_fix}] ⚡ [區間已融攝] 段落 {list(merge_tuple)} 已在前置文脈重熔中一併修復，略過重複處理。")
            continue
        ...
        if isinstance(cached_entry, str):
            ...
            covered_by_earlier_fixes.update(merge_tuple)
            continue
        ...
        if new_content:
            ...
            cached_corrections[merge_tuple] = new_content
            save_checkpoint(checkpoint_path, cached_corrections)
            covered_by_earlier_fixes.update(merge_idx)
        else:
            failed_count += 1
            logger.error(
    ...
    # ★ 關鍵改善：磁碟結構已更新，清理舊 Checkpoint，並刪除 review.json 強制進入 AI 複審驗收
    remove_checkpoint(checkpoint_path)
    if os.path.exists(review_path):
        try:
            os.remove(review_path)
            logger.info(f"🔄 段落修復完畢，已清除暫存報告，強制進入 AI 複審驗收流程（{os.path.basename(review_path)}）")
        except Exception:
            pass
...
def is_review_json_valid(review_path: str, md_path: str, segments: List[str]) -> bool:
    ...
    if not os.path.exists(review_path):
        return False
    # 若 MD 檔案在審查報告產出後有被實質修改（>2 秒），報告一律視為過期無效
    ...
    try:
        ...
        for iss in issues:
            if isinstance(iss, dict):
                m = iss.get("merge_indices") or [iss.get("index", 0)]
                for i in m:
                    if isinstance(i, int):
                        raw_indices.append(i)
                    elif isinstance(i, str) and i.strip().isdigit():
                        raw_indices.append(int(i.strip()))
...
def detect_current_state(
    ...
    dynamic_gap_threshold = max(25, int(norm_sutra_len * 0.02))

    # ★ 優先關鍵判斷：只要總覆蓋率 < 95% 或漏字超過動態閾值，優先補漏
    if coverage_pct < 95.0 or gap_chars >= dynamic_gap_threshold:
        logger.info(
            f"🔍 [狀態感知] 檢測到目前經文覆蓋率為 {coverage_pct:.1f}%（尚缺 {gap_chars} 字未銷文）。\n"
            f"   👉 智慧判定為【經文未補齊】，優先自動補齊漏段（不提前調用 AI 審查）！"
        )
    ...
    if is_review_json_valid(review_path, output_path, segments):
        ...
        if issues is not None and issues:
            ...
            # 實質漏段過濾：排除長度極短且為科判標記的縫隙
            substantive_gaps = [g for g in gaps if not is_ignorable_gap(getattr(g, "gap_text", "")) and len(normalize_text(getattr(g, "gap_text", ""))) >= 3]
            substantive_gap_chars = sum(len(normalize_text(getattr(g, "gap_text", ""))) for g in substantive_gaps)
            if substantive_gaps and substantive_gap_chars > 0:
                logger.info(f"🔍 [狀態感知] 審查通過但仍殘留 {len(substantive_gaps)} 處縫隙 ({substantive_gap_chars} 字)，進入【終局安全補漏】。")
                return PipelineState.NEED_GAP_FILL, {"gaps": substantive_gaps}
            else:
                logger.info("🔍 [狀態感知] 審查報告 0 問題且經文已無實質漏段，流程已圓滿完成！")
...
def run_pipeline(
    ...
    while True:
        ...
        clean_total_len = len(normalize_text(sutra_text))
        cur_covered_before = max(0, clean_total_len - sum(len(normalize_text(getattr(g, "gap_text", ""))) for g in cur_gaps))
        md_mtime_before = os.path.getmtime(output_path) if os.path.exists(output_path) else 0

        # ★ 防死循環保護：若審查與修復反覆震盪超過上限，直接判定未完工退出（嚴禁偽造合格報告）
        if state == PipelineState.NEED_AI_REVIEW:
            review_cycle_count += 1
            if review_cycle_count > max_cycles:
                logger.warning(
                    f"⚠️ [保護機制] AI 審查與修復已達上限 ({max_cycles} 輪)，"
                    f"仍有未通過之問題，判定未完工退出，不予自動放行！"
                )
                return False, last_review_issues
        ...
        if state == PipelineState.COMPLETED:
            ...
            if last_review_issues:
                logger.info(f"🎉 流水線處理完畢，全文覆蓋完備（但最後審查仍有 {len(last_review_issues)} 處建議待查）。")
            else:
                logger.info("🎉🎉🎉 全流程圓滿完成！經文 100% 全文覆蓋，且已通過因明文法深度審查！")
            logger.info(f"最終成果：{output_path}")
            logger.info("=" * 68)
            return True, last_review_issues
            ...
            if issues_to_fix:
                ...
                last_review_issues = issues_to_fix
        ...
        # 動作執行後重新核算進度，精確區分「生成推進停滯」與「修復停滯」
        post_segs = extract_segments_from_md(output_path) if os.path.exists(output_path) else []
        post_gaps = find_missing_gaps(sutra_text, post_segs)
        cur_covered_after = max(0, clean_total_len - sum(len(normalize_text(getattr(g, "gap_text", ""))) for g in post_gaps))
        md_mtime_after = os.path.getmtime(output_path) if os.path.exists(output_path) else 0

        is_stalled = False
        if state == last_state and state != PipelineState.COMPLETED:
            if state in (PipelineState.NEED_GENERATE, PipelineState.NEED_GAP_FILL):
                # 生成與補漏模式：字數未增加視為停滯
                is_stalled = (cur_covered_after <= cur_covered_before)
            elif state in (PipelineState.NEED_REVIEW_FIX, PipelineState.NEED_CHECKPOINT_FIX):
                # 修復模式：檔案未有任何寫入更動視為停滯
                is_stalled = (md_mtime_after == md_mtime_before)

        if is_stalled:
            stall_count += 1
            if stall_count >= 2:
                logger.error(f"\n❌ [防死鎖警報] 流水線在【{state.value}】階段連續 2 輪無實質進展，安全中止以防止無效循環。")
        ...
        last_state = state
        last_covered_chars = cur_covered_after

def find_sutra_md_pairs(
    root_dir: str,
    logger: Optional[logging.Logger] = None,
    for_generate: bool = False
) -> List[Tuple[str, str]]:
    """遞迴搜尋目錄下檔案並自動匹配經文 txt 與銷文 md（支援 --generate 純經文目錄掃描）"""
    ...
    # 忽略常見的備份、整理暫存與版本控制目錄
    ignore_dirs = {".git", ".vscode", "__pycache__", "backup", "bak", "temp", "tmp", "原文_整理"}
        ...
        for f in files:
            if f.endswith("_銷文.md") and not f.startswith("._") and not f.endswith(".bak"):
                ...
                if matched_txt:
                    ...
                    seen_txts.add(os.path.abspath(matched_txt))
                    seen_mds.add(os.path.abspath(md_path))
                else:
                    unmatched_mds.append(md_path)

        # 2. 若為全本銷文模式，額外掃描尚未建立 MD 的原始 .txt 經文
        if for_generate:
            for f in files:
                if f.lower().endswith(".txt") and not f.startswith("._") and not f.endswith(".bak"):
                    # 排除金鑰檔、日誌檔與緊急暫存檔
                    if any(kw in f.lower() for kw in ["key", "log", "checkpoint", "review", "emergency"]):
                        continue
                    txt_path = os.path.join(root, f)
                    if os.path.abspath(txt_path) not in seen_txts:
                        base_name = os.path.splitext(f)[0]
                        # 淨化後綴名稱（去除 _經文 / _原典 / _原文 等，避免重複堆疊）
                        for suffix in ["_經文", "_原典", "_原文"]:
                            if base_name.endswith(suffix):
                                base_name = base_name[:-len(suffix)]
                                break
                        md_path = os.path.join(root, f"{base_name}_銷文.md")
                        abs_md = os.path.abspath(md_path)
                        if abs_md not in seen_mds:
                            pairs.append((txt_path, md_path))
                            seen_txts.add(os.path.abspath(txt_path))
                            seen_mds.add(abs_md)
...
def run_batch(
    ...
    logger.info("=" * 70)
    logger.info("📋 掃描到的待處理檔案清單：")
    for i, (txt_p, md_p) in enumerate(pairs, 1):
        logger.info(f"   [{i:02d}/{total_files:02d}] 經文: {txt_p}  ➔  銷文: {md_p}")
    logger.info("=" * 70)

    # 讀取批次續傳狀態
    batch_checkpoint_exists = os.path.exists(BATCH_CHECKPOINT_FILE)
    batch_data = load_batch_checkpoint() if batch_checkpoint_exists else {}
    completed_set = set(batch_data.get("completed", []))
    is_resume_mode = batch_checkpoint_exists and len(completed_set) > 0

    if is_resume_mode:
        logger.info(f"⚡ [批次中斷續傳] 偵測到未完批次進度檔，已有 {len(completed_set)} 個檔案完工，將自動接續！")
        if getattr(args, "reset", False):
            logger.info("🧹 [批次中斷續傳] 偵測到 --reset：已完工檔案將繼續略過，未完工檔案將重置快取！")
    else:
    ...
    # ★ 平行處理參數（--parallel / --workers；未指定時預設關閉 = 依序逐一處理，手動傳 2 以上才啟用平行）
    workers = max(1, int(getattr(args, "parallel", None) or 1))
    if workers > 1:
    ...
    if workers <= 1:
        ...
        # ★ 平行模式：動態滑動窗口提交，已完工檔案瞬間跳過，未完工檔案保持滿額 5 路並行
        pending: Dict[Any, int] = {}
        try:
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for actual_idx, (txt_path, md_path) in enumerate(pairs, 1):
                    md_abs = os.path.abspath(md_path)

                    # 批次中斷續傳跳過機制：已完工檔案瞬間略過，不阻塞主執行緒
                    if md_abs in completed_set:
                        logger.info(f"⚡ [批次跳過 {actual_idx:02d}/{total_files:02d}] 檔案先前已完工，直接略過：{os.path.basename(md_path)}")
                        success_files.append(md_path)
                        continue

                    # 批次熔斷檢查：所有金鑰耗盡時停止派發新任務
                    if key_pool and key_pool.is_all_dead():
                        logger.critical("\n🛑 [批次熔斷] 所有 API 金鑰已全數耗盡欠費！停止提交新檔案。")
                        break

                    # 若當前併發任務已達 workers 上限，等待至少一個任務完成再繼續派發
                    ...
                    # 派發未完工檔案至工作緒池
                    fut = executor.submit(process_file, actual_idx, txt_path, md_path)
                    pending[fut] = actual_idx
            ...
            # ★ 此刻 with 區塊已 shutdown(wait=True) 等待所有 worker 收工，成果皆已落盤；
            #   將所有尚未記錄的結果逐一補記，中斷後已完工檔案不會被重跑
    ...
    if len(success_files) == total_files:
        remove_batch_checkpoint()
        logger.info("🧹 [批次完成] 全數檔案皆已圓滿處理完畢，已自動清理批次進度暫存檔。")
    ...
    if failed_files:
        ...
        for f_path, reason in failed_files:
            # 優先顯示包含父資料夾的相對路徑，避免 2_銷文.md 重複混淆
            rel_display = os.path.relpath(f_path) if os.path.isabs(f_path) else f_path
            logger.info(f"   - {rel_display}: {reason}")

    if files_with_unresolved_issues:
        logger.warning("\n" + "!" * 70)
        logger.warning(f"⚠️ 【注意】共有 {len(files_with_unresolved_issues)} 個檔案在最後審查輪次仍有問題待處理：")
        for item in files_with_unresolved_issues:
            f_name = item["file"]
            f_issues = item["issues"]
            logger.warning(f"\n📄 檔案：{f_name}（共 {len(f_issues)} 處待處理問題）")
            for iss_idx, iss in enumerate(f_issues, 1):
                m_str = ",".join(map(str, iss.get("merge_indices", [iss.get("index", 0)])))
                iss_type = iss.get("type") or iss.get("issue_type") or "未知問題"
                prob = iss.get("problem", "")
                logger.warning(f"   [{iss_idx:02d}] 段落 [{m_str}] ({iss_type})：{prob}")
        logger.warning("!" * 70)
    else:
        logger.info("\n✨ 所有檔案皆無殘留問題，全數完美通過審查！")
    logger.info("=" * 70)
...
def auto_output_path(input_file: str) -> str:
    """自動推導 MD 檔路徑（自動淨化 _經文/_原典/_銷文 後綴，防止重複堆疊）"""
    input_abs = os.path.abspath(input_file)
    input_dir = os.path.dirname(input_abs)
    base_name = os.path.splitext(os.path.basename(input_abs))[0]
    for suffix in ["_經文", "_原典", "_原文", "_銷文"]:
        if base_name.endswith(suffix):
            base_name = base_name[:-len(suffix)]
            break
    return os.path.join(input_dir, f"{base_name}_銷文.md")
...
def main():
    ...
    parser.add_argument("--parallel", "--workers", type=int, default=None, help="★ 平行處理工作緒數（僅遞迴批次模式生效；預設關閉 = 依序逐一處理，手動傳 2 以上即多檔同時校對）")
    ...
    parser.add_argument("--reasoning-effort", type=str, default="max", choices=["low", "medium", "high", "max"])
    parser.add_argument("--max-review-cycles", type=int, default=2, help="AI 審查與修復的最大交替輪次（防死循環，建議設為 2）")
    ...
    is_batch_mode = (args.recursive is not None)
    ...
    if getattr(args, "glm53_flash", False):
        ...
        if "stealth" in ox_val.lower() or ox_val.lower() in ["or", "openrouter"]:
            ...
            if args.provider not in ["opencode", "zen"]:
                args.provider = "zen"
        else:
            args.model = ox_val
            if args.provider not in ["opencode", "zen", "openrouter"]:
                args.provider = "zen"
    ...
    key_pool = load_api_keys(
        ...
        is_openrouter=(args.provider in ["free_glm", "openrouter"]),
    ...
    # 批次遞迴處理路由（支援多目錄掃描與跨目錄去重）
    if is_batch_mode:
        target_dirs = args.recursive if len(args.recursive) > 0 else ["."]
        all_pairs = []
        seen_mds = set()

        for d in target_dirs:
            search_dir = os.path.abspath(d)
            if not os.path.exists(search_dir):
                logger.warning(f"⚠️ 指定的目錄不存在，已跳過：{search_dir}")
                continue
            pairs = find_sutra_md_pairs(search_dir, logger=logger, for_generate=(args.generate or args.auto))
            for txt_p, md_p in pairs:
                abs_md = os.path.abspath(md_p)
                if abs_md not in seen_mds:
                    seen_mds.add(abs_md)
                    all_pairs.append((txt_p, md_p))

        if not all_pairs:
            logger.warning(f"⚠️ 在指定的所有目錄中未找到任何匹配的經文與銷文檔案組合！")
            return
        run_batch(args, client, model_name, all_pairs, logger)
        return
    ...
    # 單檔模式路由
    if args.auto:
        is_ok, remaining_issues = run_pipeline(args, client, model_name, sutra_text, style, args.output, logger)
        if remaining_issues:
            logger.warning("\n" + "!" * 70)
            logger.warning(f"⚠️ 【注意】本檔案在最後審查輪次仍殘留 {len(remaining_issues)} 處問題：")
            for iss_idx, iss in enumerate(remaining_issues, 1):
                m_str = ",".join(map(str, iss.get("merge_indices", [iss.get("index", 0)])))
                iss_type = iss.get("type") or iss.get("issue_type") or "未知問題"
                prob = iss.get("problem", "")
                logger.warning(f"   [{iss_idx:02d}] 段落 [{m_str}] ({iss_type})：{prob}")
